...
class BlockPuzzleGame(private val random: Random = Random.Default, val itemsEnabled: Boolean = false) {
    ...
    fun reset() {
        board.fill(0); score=0; lines=0; gameOver=false; paused=false; bag.clear();slowPieces=0;itemMessage=null;pieces=0;clearEvent=0;lastClearedRows=emptyList()
    ...
    private fun lock() {
        ...
        pieces++;val oldLines=lines;val fullRows=(0 until height).filter{r->(0 until width).all{board[r*width+it]!=0}}
