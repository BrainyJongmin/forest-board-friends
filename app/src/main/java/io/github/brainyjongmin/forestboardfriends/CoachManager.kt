...
class CoachManager(private val context: Context) : TextToSpeech.OnInitListener {
    ...
    private fun respond(name:String,game:String,question:String,fact:String,spontaneous:Boolean,done:(String)->Unit){
        ...
        scope.launch{inference.withLock{try{val e=ensureEngine();val recent=history.takeLast(6).joinToString("\n");val config=ConversationConfig(systemInstruction=Contents.of("너는 ${name}의 다정하고 장난기 있는 숲속 여우 친구야. 자연스러운 한국어 반말로 질문의 핵심부터 답해. 매번 이름을 부르거나 칭찬하거나 질문으로 끝내지 말고, 짧은 대답·맞장구·농담·설명을 상황에 맞게 섞어. 상대가 쓴 표현과 최근 대화를 기억해서 사람 친구처럼 이어 가되 같은 문구를 반복하지 마. 게임 얘기만 강요하지 말고 학교생활, 가족, 음식, 취미, 지식, 상상놀이도 편하게 이야기해. 모르면 꾸며내지 말고 솔직히 말해. '아이', '사용자', '좋아합니다' 같은 보고체는 쓰지 말고 보통 1~4문장으로 말해. 개인정보를 묻거나 기억하지 말고 위험하거나 성인용인 내용은 보호자에게 물어보도록 안내해."),samplerConfig=SamplerConfig(topK=50,topP=.92,temperature=.55));val conversation=e.createConversation(config);val prompt=if(spontaneous)"우리는 $game 중이야. 현재 상태: $fact\n직전 말과 겹치지 않게 지금 어울리는 짧은 감탄이나 응원을 한마디만 해." else "최근 대화:\n${recent.ifBlank{"아직 없음"}}\n참고할 게임 상태(질문과 관계없으면 무시): $game, $fact\n$name: $question\n여우:";val out=StringBuilder();try{conversation.sendMessageAsync(prompt).catch{throw it}.collect{out.append(it.toString())}}finally{conversation.close()};val text=clean(out.toString()).ifBlank{"잘 못 들었어. 한 번만 더 말해 줄래?"};if(!spontaneous){history+="$name: $question\n여우: $text";while(history.size>8)history.removeAt(0)};done(text);speak(text)}catch(e:Exception){status="AI 코치 오류: ${e.message}";val text="잠깐 생각이 꼬였어. 한 번만 더 말해 줄래?";done(text);speak(text)}}}
    }
